...
public class PreferencesLoader {
     ...
     */
    public static void importPreferences(Reader in, boolean overwritePrefs) throws IOException {
     ...
     */
    public static void exportPreferences(Preferences node, Writer out) throws IOException, BackingStoreException {
        ...
        for (int i=0; i<keys.length; i++) {
            ...
            out.write(path);
            out.write('.');
            out.write(encode(keys[i],false));
            out.write('=');
            out.write(node.get(keys[i], ""));
            out.write('\n');
        }
     ...
     * @throws BackingStoreException  in case of  any errors with preferences backing store.
     */
    public static void exportPreferences(Preferences node, Map<String,String> result, boolean skipPrefix) throws BackingStoreException {
    ...
    private static void exportPreferences(Preferences node, Map<String,String> result, int skippedPrefixLength) throws BackingStoreException {
     ...
     */
    public static Preferences getPreferenceNode(String expr, boolean ensureExists) throws BackingStoreException {
    ...
    private static Preferences importNode(String line, boolean overwritePrefs) {
        line = line.trim();
    ...
    private static void processExport(String expr, File file) throws IOException, BackingStoreException {
//        System.out.println("export:  file:"+file+", prefix:"+prefix);
        if (expr==null || expr.length()==0) {
            System.out.println("root preference node not specified!");
            return;
        }
        final Writer out = file!=null ? new FileWriter(file) : new OutputStreamWriter( System.out );
        try {
    ...
    private static void processClean(String expr) {
//        System.out.println("clean:  prefix:"+prefix);
    ...
    private static void processImport(boolean overwrite, File file) throws IOException {
//        System.out.println("import file: "+file+"    "+overwrite);
        final Reader in = file!=null ? new FileReader(file) : new InputStreamReader(System.in);
        try{
